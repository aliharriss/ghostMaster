    ...
    override fun processImage() {
        ...
        runInBackground(
                Runnable {
                    ...
                    // Somehow vertically inverts the bitmap to deal with selfie and the screen orientation
                    val cx = croppedBitmap!!.width / 2f
                    val cy = croppedBitmap!!.height / 2f
                    val croppedBitmap = croppedBitmap!!.flip(1f, -1f, cx, cy)

                    val results = detector!!.recognizeImage(croppedBitmap!!)
                    lastProcessingTimeMs = SystemClock.uptimeMillis() - startTime

                    cropCopyBitmap = Bitmap.createBitmap(croppedBitmap!!)
                    val newCanvas = Canvas(croppedBitmap!!)
                    ...
                    for (result in results) {
                        ...
                        if (result.confidence >= minimumConfidence && result.title == "person") {
